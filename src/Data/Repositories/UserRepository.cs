...
namespace Data.Repositories
{
    public class UserRepository : BaseRepository<User>, IUserRepository
    {
        ...
        public async Task<IEnumerable<User>> GetFollowers(Guid userId)
            => await _context.Follows.Where(f => f.FollowedId == userId).Select(f => f.Follower).ToListAsync();

        public async Task<IEnumerable<User>> GetFollowing(Guid userId)
            => await _context.Follows.Where(f => f.FollowerId == userId).Select(f => f.Followed).ToListAsync();
        ...
        public async Task<IEnumerable<User>> SearchUsers(string query)
        {
            ...
            return await _context.Users
                .Where(u => u.Username.ToLower().Contains(query) || u.DisplayName.ToLower().Contains(query))
